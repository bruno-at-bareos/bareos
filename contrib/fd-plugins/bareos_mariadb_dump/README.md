...
This plugin makes a backup of each database found in mariadb in a single file.
For restore select the needed database file, found in `@mariabackup@` in the catalog.
    ...
    Name = "client-data"
       Include  {
                Plugin = "python:"
                         ...
                         "module_name=bareos_mariadb_dump:"
        }
}
```
...
```
    Plugin = "python:module_path=/usr/lib64/bareos/plugins:module_name=bareos-fd-mariadb:db=test,mariadb"
```
...
Command (with or without full path) to create the dumps. Default: mariadb-dump
